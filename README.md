...
Install Node.js to run Web application

To install all the Dependecies and imported libraries command below npm i

To install ionic components run below command: npm install -g ionic

To Run web application ionic serve

FOR MOBILE APPLICATION
To work with Ionic we need following set of tools to be installed in your machine.

Android SDK (https://developer.android.com/studio/)
js (https://nodejs.org ) Once android studio is downloaded please install it with admin permission.
...
Open your node.js comment prompt and install IONIC Plugins by using the below command
npm install -g cordova ionic Once required node packages are downloaded for ionic we are set to go to create ionic app.

Build IONIC by using command below ionic cordova platform add android
